...
const ProjectTile = ({ slug, title, description }) => (
  <div className="col-span-full sm:col-span-1 flex flex-col p-6 bg-white border border-stone-200 rounded-lg dark:bg-gray-800 dark:border-gray-700">
    ...
    <Link href={`/projects/${slug.replace(/\.mdx?$/, "")}`} passHref>
      <a className="flex justify-center items-center p-3 bg-pebble border border-pebble-dark rounded-full font-medium transition hover:bg-pebble-dark dark:bg-gray-700 dark:border-gray-600 dark:hover:bg-gray-600 focus:outline-none focus:ring focus:ring-orange">
        Read more
      </a>
    </Link>
  </div>
